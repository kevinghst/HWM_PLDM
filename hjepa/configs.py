...
class DataclassArgParser(argparse.ArgumentParser):
    ...
    def _populate_dataclass_from_dict(dtype: DataClassType, d: dict):
        ...
        keys = {f.name: f.type for f in dataclasses.fields(dtype)}
        inputs = {k: v for k, v in d.items() if k in keys}

        for k, v in inputs.items():
            field_type = keys[k]

            # Handle Union types (e.g., Union[EnumType, None])
            ...
            # Check if the type is a list (e.g., List[EnumType])
            if get_origin(field_type) in (list, List):
                elem_type = get_args(field_type)[0]
                # Handle Union in the list element type
                if get_origin(elem_type) is Union:
                    args = get_args(elem_type)
                    non_none_types = [t for t in args if t is not type(None)]
                    if len(non_none_types) == 1:
                        elem_type = non_none_types[0]
                if isinstance(elem_type, type) and issubclass(elem_type, Enum):

                    def instantiate_enum(item):
                        if isinstance(item, str):
                            try:
                                # Lookup enum member by its name
                                return elem_type[item]
                            except KeyError:
                                # Fallback: if lookup by name fails, try by value
                                return elem_type(item)
                        return item

                    if isinstance(v, list):
                        inputs[k] = [instantiate_enum(item) for item in v]
                    else:
                        inputs[k] = instantiate_enum(v)
            # For single enum fields (not in a list)
