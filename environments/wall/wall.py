...
from environments.wall.data.wall_utils import check_wall_intersect
...
class DotWall(gym.Env):
    ...
    def start_goal_shortest_distance(self):
        """
        Calculate the shortest distance from start to goal through the door
        """
        init_pos = self.position_history[0].cpu()
        crossing_pos = torch.tensor([self.wall_x, self.hole_y]).cpu()

        shorest_distance = torch.norm(init_pos - crossing_pos) + torch.norm(
            crossing_pos - self.target_position.cpu()
        )
    ...
    def reset(
        ...
        if dot_position is None:
            ...
            self.dot_position = dot_position.to(self.device)
            assert target_position is not None
            self.target_position = target_position.to(self.device)
    ...
    def step(self, action: np.array) -> Tuple[ObsType, float, bool, bool, InfoType]:
        ...
        return obs, done.cpu().numpy(), done, truncated, self._build_info()
